 ...
 */
export function x402TranslationMiddleware(config: X402TranslationConfig): MiddlewareHandler {
	return async (c, next) => {
		...
		if (c.res.status === 200) {
			...
			if (x402Response) {
				try {
					const x402Receipt = JSON.parse(base64urlDecode(x402Response)) as X402PaymentReceipt
...
function getAssetAddress(asset: string | undefined, network: string): Address {
	...
	return networkAssets[asset ?? 'USDC'] ?? '0x20c0000000000000000000000000000000000001'
}
