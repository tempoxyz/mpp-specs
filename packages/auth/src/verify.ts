...
export async function verifyRequest(
	...
	try {
		const publicKey =
			typeof keyInfo.publicKey === 'string'
...
function parseSignatureInput(input: string): ParsedSignatureInput | null {
	...
	if (label === undefined || componentsPart === undefined) {
		return null
	}
	...
	if (paramsPart) {
		...
		for (const matchResult of paramMatches) {
			const key = matchResult[1]
			const strVal = matchResult[2]
			const numVal = matchResult[3]

			if (!key) continue

			if (key === 'created' || key === 'expires') {
				const value = numVal ?? strVal
				if (value !== undefined) {
					params[key] = parseInt(value, 10)
				}
			} else if (key === 'keyid' || key === 'nonce' || key === 'alg') {
				if (strVal !== undefined) {
					params[key] = strVal as Algorithm
				}
			}
		}
	}
...
function getVerifyAlgorithm(algorithm: Algorithm): Parameters<typeof crypto.subtle.verify>[0] {
	switch (algorithm) {
		case 'ed25519':
			return { name: 'Ed25519' }
...
async function importPublicKey(key: string, algorithm: Algorithm): Promise<CryptoKey> {
	...
	return crypto.subtle.importKey('spki', keyData, keyAlgorithm, true, ['verify'])
}
