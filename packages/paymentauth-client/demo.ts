...
const debug = (msg: string) => {
  if (config.verbose)
    console.error(`${colors.dim}[debug]${colors.reset} ${msg}`);
};
const info = (msg: string) =>
  console.error(`${colors.blue}▶${colors.reset} ${msg}`);
const success = (msg: string) =>
  console.error(`${colors.green}${colors.reset} ${msg}`);
...
async function main() {
  ...
  if (body)
    debug(`Body: ${body.slice(0, 100)}${body.length > 100 ? "..." : ""}`);
  ...
  if (response.status !== 402) {
    ...
    try {
      ...
      console.log(
        config.verbose ? JSON.stringify(json, null, 2) : JSON.stringify(json)
      );
    } catch {
      console.log(text);
    }
  ...
  if (!privateKey)
    throw error(
      "PRIVATE_KEY required. Set PRIVATE_KEY=0x... environment variable."
    );
  ...
  info(
    `Payment: ${amountFormatted} USD to ${asset.slice(0, 10)}...${asset.slice(
      -6
    )}`
  );
