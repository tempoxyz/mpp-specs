import { TempoSigner, signRequest, computeContentDigest } from '@tempo/auth'
import type {
  Transaction,
...
class TransactionsAPI {
  ...
  async list(input: ListTransactionsInput = {}): Promise<ListTransactionsResponse> {
    ...
    const response = await this.client.request<ListTransactionsResponse>(
      'GET',
      path
    )
    return response.data
...
class AccessKeysAPI {
  ...
  async get(id: string): Promise<AccessKey> {
    const response = await this.client.request<AccessKey>(
      'GET',
