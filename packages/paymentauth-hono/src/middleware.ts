 ...
 */
function createChallenge(config: PaymentAuthConfig): PaymentChallenge<ChargeRequest> {
 ...
 * @example
 * ```ts
 * import { paymentAuth } from '@tempo/paymentauth-hono'
 *
...
export function paymentAuth(config: PaymentAuthConfig): MiddlewareHandler {
	return async (c: Context, next) => {
		...
		if (!storedChallenge) {
			c.header('WWW-Authenticate', formatWwwAuthenticate(createChallenge(config)))
			return c.json(
		...
		const verification = await config.verify(signedTx, storedChallenge.challenge.request)
