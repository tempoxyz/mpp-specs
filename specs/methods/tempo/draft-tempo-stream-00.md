...
Signature Type
: Tempo transactions support multiple signature schemes identified by
  type bytes: secp256k1 (0x00), P256 (0x01), WebAuthn (0x02), and
  Keychain (0x03).
...
| signature | variable | See {{signature-format}} for encoding per signature type |
...
Tempo transactions support multiple signature schemes to accommodate
...
|---------|------|-------------|
| (none) | secp256k1 | Standard ECDSA on secp256k1 curve |
| `0x01` | P256 | ECDSA on NIST P-256 curve |
| `0x02` | WebAuthn | WebAuthn/FIDO2 signatures with authenticator data |
| `0x03` | Keychain | Access key signing on behalf of root account |

Note: secp256k1 signatures have no type identifier prefix and are
detected by their fixed 65-byte length for backward compatibility.

## Secp256k1 Signature

Standard Ethereum-style ECDSA signature on the secp256k1 curve.

~~~
+-------+-------+-------+
|   r   |   s   |   v   |
+-------+-------+-------+
   32      32      1      = 65 bytes total
~~~
...
| `v` | 1 byte | Recovery identifier (27 or 28, or 0/1) |

The signer address is recovered using standard ecrecover.

## P256 Signature

ECDSA signature on the NIST P-256 curve (secp256r1), used by Apple
Secure Enclave, Android Keystore, and passkeys.

~~~
+------+-------+-------+-----------+-----------+------+
| Type |   r   |   s   | pubkey_x  | pubkey_y  | flag |
+------+-------+-------+-----------+-----------+------+
   1      32      32        32          32        1     = 130 bytes total
~~~
...
| `r` | 32 bytes | ECDSA signature r component |
| `s` | 32 bytes | ECDSA signature s component |
| `pubkey_x` | 32 bytes | Public key X coordinate |
| `pubkey_y` | 32 bytes | Public key Y coordinate |
| `flag` | 1 byte | Pre-hash flag (`0x00` or `0x01`) |

The `s` value MUST be in low-s form (s ≤ n/2) to prevent signature
malleability. The signer address is derived as:
...
WebAuthn signatures include authenticator data and client data per
{{WebAuthn}}, enabling FIDO2 hardware keys and platform authenticators.

~~~
+------+-------------------+-------+-------+-----------+-----------+
| Type | authenticatorData |   r   |   s   | pubkey_x  | pubkey_y  |
+------+-------------------+-------+-------+-----------+-----------+
   1         37+ bytes        32      32        32          32
~~~

| Field | Size | Description |
|-------|------|-------------|
| Type | 1 byte | `0x02` |
| `authenticatorData` | 37+ bytes | Per WebAuthn spec |
| `clientDataJSON` | variable | JSON with challenge |
| `r` | 32 bytes | P256 ECDSA signature r component |
| `s` | 32 bytes | P256 ECDSA signature s component |
| `pubkey_x` | 32 bytes | Public key X coordinate |
| `pubkey_y` | 32 bytes | Public key Y coordinate |
...
| Field | Size | Description |
|-------|------|-------------|
| `rpIdHash` | 32 bytes | SHA-256 hash of relying party ID |
| `flags` | 1 byte | Bit flags (UP, UV, AT, ED) |
| `signCount` | 4 bytes | Signature counter (big-endian) |

Flag requirements:

- UP (bit 0) or UV (bit 2) MUST be set
- AT (bit 6) MUST NOT be set (assertion, not attestation)
- ED (bit 7) MUST NOT be set (extensions not supported)

The `clientDataJSON` MUST contain:

~~~json
{
  "type": "webauthn.get",
  "challenge": "<base64url(transaction_signing_hash)>",
  "origin": "<origin>"
}
~~~
...
Keychain signatures enable access keys to sign on behalf of a root
account, supporting delegation without requiring the root key.

~~~
+------+--------------+------------------+
| Type | user_address | inner_signature  |
+------+--------------+------------------+
   1        20           65-2049 bytes
~~~
...
|-------|------|-------------|
| Type | 1 byte | `0x03` |
| `user_address` | 20 bytes | Root account address |
| `inner_signature` | variable | Secp256k1, P256, or WebAuthn signature |

The `inner_signature` MUST NOT be another Keychain signature (no
recursive wrapping). The access key relationship MUST be verified
onchain.
...
1. **Signature type detection**: The contract MUST support all signature
   types defined in {{signature-format}}. Signature type is determined by
   inspecting the first byte and length of the signature data.

2. **Canonical signatures**: For ECDSA signatures (secp256k1 and P256),
   the contract MUST reject signatures with non-canonical (high-s) values.
   Signatures MUST have `s <= curve_order / 2`. See {{signature-malleability}}
   for rationale.

3. **Authorized signer verification**: The contract MUST recover the
   signer address from the signature and verify it matches the expected
...
4. **Domain binding**: The contract MUST use its own address as the
...
a voucher signature. Voucher signatures use the formats defined in
{{signature-format}}. Examples use abbreviated values for brevity; the
voucher structure is shown in full in the type="hash" example above.
...
4. Sign using any supported signature type (see {{signature-format}})
...
length checks) before expensive signature verification to minimize
...
signatures with "low-s" values where `s <= curve_order / 2`.

This requirement applies to all ECDSA-based signature types:

- **secp256k1**: Half-order is
  `0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0`
- **P256**: Half-order is
  `0x7FFFFFFF800000007FFFFFFFFFFFFFFFDE737D56D38BCF4279DCE5617E3192A8`

Servers MUST reject signatures with `s` values exceeding the applicable
threshold. See {{signature-format}} for supported signature types and
their encoding.

## Signature Verification

Servers MUST verify signatures according to their type:

**Secp256k1**: Standard ecrecover verification. The recovered address
MUST match the expected payer or authorized signer.

**P256**: Servers MUST verify:

1. The signature (r, s) is valid for the public key (pubkey_x, pubkey_y)
2. The `s` value is in low-s form (s ≤ n/2) per {{SEC1}}
3. The derived address matches the expected payer or authorized signer

**WebAuthn**: Servers MUST verify per {{WebAuthn}}:

1. The `type` in clientDataJSON equals `"webauthn.get"`
2. The `challenge` in clientDataJSON matches the signing hash
3. The UP or UV flag is set in authenticatorData
4. The AT and ED flags are NOT set
5. The P256 signature is valid over SHA-256(authenticatorData || SHA-256(clientDataJSON))

**Keychain**: Servers MUST verify:

1. The inner signature is valid (not another Keychain)
2. The access key relationship exists onchain between the signing key
   and the user_address
...
; Signature: variable length depending on type (see Signature Format section)
; secp256k1: 65 bytes, P256: 130 bytes, WebAuthn: variable, Keychain: variable
signature = tstr .regexp "0x[0-9a-fA-F]+"
...
signed-voucher = {
  ...
  signature: signature,  ; see Signature Format section for encoding
}
    ...
    "signedTypedData": {
      ...
      "properties": {
        ...
        "signature": {
          "type": "string",
          "pattern": "^0x[0-9a-fA-F]+$",
          "description": "Signature in one of the supported formats (see Signature Format section)"
        }
      }
    }
  }
