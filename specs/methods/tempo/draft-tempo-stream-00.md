...
| `signature` | string | REQUIRED | Hex-encoded signature (see {{signature-format}}) |
...
3. Verify the initial voucher:
    - Verify signature per {{signature-format}}
...
1. Verify voucher signature per {{signature-format}}
2. Verify signature uses canonical low-s values (see {{signature-malleability}})
3. Verify signer matches expected signer from on-chain state
