...
HTTP Authentication Scheme {{PAYMENT-AUTH}}. It specifies how
...
of {{PAYMENT-AUTH}}. It defines the `request` and `payload`
...
per Section 13.4 of {{PAYMENT-AUTH}}.
...
JSON object per Section 5.2 of {{PAYMENT-AUTH}}.
...
per Section 5.3 of {{PAYMENT-AUTH}}.
...
Methods" registry established by {{PAYMENT-AUTH}}:
...
|--------|-------------------|-------------|-----------|
| `charge` | `tempo` | One-time TIP-20 transfer | This document, {{intent-charge}} |
| `authorize` | `tempo` | Payment authorization | This document, {{intent-authorize}} |
| `subscription` | `tempo` | Recurring payment authorization | This document, {{intent-subscription}} |

Note: `charge` is already registered as a base intent by
{{PAYMENT-AUTH}}. This document extends its definition for
the `tempo` method.
