...
async function main() {
	...
	const challengeRes = await fetch(`${API_URL}/move/${action}`, { method: 'POST' })
	...
	console.log(`Amount: ${challenge.request.amount} (${Number(challenge.request.amount) / 1e6} USD)`)
...
function parseWwwAuthenticate(header: string) {
	...
	if (params.request) {
		params.request = JSON.parse(Buffer.from(params.request, 'base64').toString())
	}
	...
	return params as unknown as {
		...
		method: string
		intent: string
		request: { amount: string; asset: string; destination: string; expires: string }
		expires: string
		description: string
	}
}
