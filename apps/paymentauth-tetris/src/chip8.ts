	...
	private execute(opcode: number): void {
		...
		switch (opcode & 0xf000) {
			...
			case 0x5000: // SE Vx, Vy - Skip if Vx == Vy
				if ((this.V[x] ?? 0) === (this.V[y] ?? 0)) this.pc += 2
				break
			...
			case 0x8000:
				switch (n) {
					case 0x0: // LD Vx, Vy
						...
						break
					case 0x1: // OR Vx, Vy
						this.V[x] = (this.V[x] ?? 0) | (this.V[y] ?? 0)
						break
					case 0x2: // AND Vx, Vy
						this.V[x] = (this.V[x] ?? 0) & (this.V[y] ?? 0)
						break
					...
					case 0x4: {
						...
						const sum = (this.V[x] ?? 0) + (this.V[y] ?? 0)
						this.V[0xf] = sum > 255 ? 1 : 0
						this.V[x] = sum & 0xff
						break
					}
					case 0x5: // SUB Vx, Vy (with borrow)
						this.V[0xf] = (this.V[x] ?? 0) > (this.V[y] ?? 0) ? 1 : 0
						this.V[x] = ((this.V[x] ?? 0) - (this.V[y] ?? 0)) & 0xff
						break
					case 0x6: // SHR Vx
						this.V[0xf] = (this.V[x] ?? 0) & 0x1
						this.V[x] = (this.V[x] ?? 0) >> 1
						break
					case 0x7: // SUBN Vx, Vy
						this.V[0xf] = (this.V[y] ?? 0) > (this.V[x] ?? 0) ? 1 : 0
						this.V[x] = ((this.V[y] ?? 0) - (this.V[x] ?? 0)) & 0xff
						break
					case 0xe: // SHL Vx
						this.V[0xf] = ((this.V[x] ?? 0) & 0x80) >> 7
						this.V[x] = ((this.V[x] ?? 0) << 1) & 0xff
						break
				}
				break
			...
			case 0xd000: {
				...
				const xPos = (this.V[x] ?? 0) % DISPLAY_WIDTH
				const yPos = (this.V[y] ?? 0) % DISPLAY_HEIGHT
				this.V[0xf] = 0

				for (let row = 0; row < n; row++) {
					const sprite = this.memory[this.I + row] ?? 0
					for (let col = 0; col < 8; col++) {
						if ((sprite & (0x80 >> col)) !== 0) {
							...
							if ((this.display[idx] ?? 0) === 1) {
								this.V[0xf] = 1 // Collision
							}
							this.display[idx] = (this.display[idx] ?? 0) ^ 1
						}
					}
				}
				break
			...
			case 0xe000:
				switch (nn) {
					case 0x9e: // SKP Vx - Skip if key Vx is pressed
						if ((this.keys[this.V[x] ?? 0] ?? 0) === 1) this.pc += 2
						break
			...
			case 0xf000:
				switch (nn) {
					...
					case 0x1e: // ADD I, Vx - Set I = I + Vx
						this.I = (this.I + (this.V[x] ?? 0)) & 0xfff
						break
					case 0x29: // LD F, Vx - Set I = sprite location for digit Vx
						this.I = ((this.V[x] ?? 0) & 0xf) * 5
						break
					case 0x33: // LD B, Vx - Store BCD of Vx at I, I+1, I+2
						this.memory[this.I] = Math.floor((this.V[x] ?? 0) / 100)
						this.memory[this.I + 1] = Math.floor(((this.V[x] ?? 0) % 100) / 10)
						this.memory[this.I + 2] = (this.V[x] ?? 0) % 10
						break
					case 0x55: // LD [I], Vx - Store V0-Vx at I
						for (let i = 0; i <= x; i++) {
							this.memory[this.I + i] = this.V[i] ?? 0
						}
						break
					case 0x65: // LD Vx, [I] - Load V0-Vx from I
						for (let i = 0; i <= x; i++) {
							this.V[i] = this.memory[this.I + i] ?? 0
						}
						break
				}
				break
