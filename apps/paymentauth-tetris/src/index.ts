...
async function verifyWebAuthnSignature(
	...
	try {
		...
		const verified = await WebAuthnP256.verify({
			challenge: OxHex.fromString(challengeId),
			signature: {
				r: BigInt(payload.signature.r),
				s: BigInt(payload.signature.s),
			},
			metadata: {
				authenticatorData: payload.metadata.authenticatorData,
				clientDataJSON: payload.metadata.clientDataJSON,
				challengeIndex: payload.metadata.challengeIndex,
				typeIndex: payload.metadata.typeIndex,
			},
...
app.post('/move/:action', async (c) => {
	...
	if (!['left', 'right', 'rotate', 'drop'].includes(action)) {
		return c.json({ error: 'Invalid action. Use: left, right, rotate, drop' }, 400)
	}
	...
	if (!authHeader || !authHeader.startsWith('Payment ')) {
		...
		return c.json(new PaymentRequiredError('Payment required to make a move').toJSON(), 402)
	}
	...
	try {
		...
		return c.json(new MalformedProofError('Invalid Authorization header').toJSON(), 400)
	}
	...
	if (!storedChallenge) {
		...
		return c.json(new PaymentVerificationFailedError('Unknown challenge ID').toJSON(), 401)
	}

	if (storedChallenge.used) {
		c.header('WWW-Authenticate', formatWwwAuthenticate(createChallenge(c.env)))
		return c.json(new PaymentVerificationFailedError('Challenge already used').toJSON(), 401)
	}
	...
	if (payloadType === 'webauthn') {
		...
		const signResult = await signPaymentTransaction(storedChallenge.challenge.request, c.env)
		if (signResult.error) {
			return c.json(new PaymentVerificationFailedError(signResult.error).toJSON(), 500)
		}
		...
		if (!verification.valid) {
			return c.json(
				new PaymentVerificationFailedError(verification.error || 'Verification failed').toJSON(),
				400,
			)
		}
