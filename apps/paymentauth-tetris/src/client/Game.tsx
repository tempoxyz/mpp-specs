...
function parseWwwAuthenticate(header: string): PaymentChallenge {
	...
	if (params.request) {
		;(params as Record<string, unknown>).request = JSON.parse(base64urlDecode(params.request))
	}
	...
	const makeMove = useCallback(
		async (action: TetrisAction) => {
			...
			try {
				...
				const authHeader = `Payment ${base64urlEncode(JSON.stringify(credential))}`
		...
		<div className="game-container">
			<div className="frame">
				...
				<div className="display-container">
					<pre className="display">{loading ? 'loading...' : (gameState?.ascii ?? 'no data')}</pre>
				</div>
