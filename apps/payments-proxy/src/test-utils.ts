 ...
 */
export function createMockContext(req: Request, env: Env): Context<{ Bindings: Env }> {
	return {
		req: {
			raw: req,
			url: req.url,
 ...
 */
export function createMockPartner(overrides: Partial<PartnerConfig> = {}): PartnerConfig {
	return {
		name: 'Test Partner',
		slug: 'test',
 ...
 */
export function createMockRpcError(code: number, message: string, id = 1): Response {
	return new Response(
		JSON.stringify({
			jsonrpc: '2.0',
			id,
 ...
 */
export function extractChallengeId(wwwAuth: string | null | undefined): string | null {
